...
point <- define_wells(tibble::tibble(wID=1,x=0,y=0,Q=0.5,R=5,diam=0.75,path=1,origin=1,w_group="a"))
...
wells <- define_wells(x=c(5,0.5),y=c(2.5,0.25),Q=c(0.5,-0.2),R=10,diam=c(0.75,0.8),w_group=c("a","b"))
...
test_that("generate_image_wells returns the proper data.frame shape",{
  expect_equal(dim(generate_image_wells(wells,bounds)),c(18,15))
  expect_equal(dim(generate_image_wells(wells,bounds)),c(50,15))
})
