...
case3 <- tibble::tibble(Well=c("C1","C2","D","C3"),
                        Baseline=c(48.305,46.666,48.39,48.967)) %>%
  dplyr::mutate(Minimum=Baseline* 0.95,
                `Head (Q = -4)`=c(47.241,45.006,44.548,48.572),
                `Head (Q = -5)`=c(46.975,44.592,43.588,48.473),
                `Head (Q = -6)`=c(46.708,44.177,42.627,48.374)) %>%
  dplyr::mutate(`% change (Q = -4)`=round((Baseline-`Head (Q = -4)`)/Baseline * 100,2),
                `% change (Q = -5)`=round((Baseline-`Head (Q = -5)`)/Baseline * 100,2),
                `% change (Q = -6)`=round((Baseline-`Head (Q = -6)`)/Baseline * 100,2)) %>%
  dplyr::arrange(Well) %>% dplyr::filter(!grepl("D",Well)) %>% select(-contains("Q = -4"),-contains("Head"))
