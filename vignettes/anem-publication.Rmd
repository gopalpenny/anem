...
According to Darcy's law, flow occurs in the direction opposite the gradient of the hydraulic head. Because hydraulic head follows the principle of superposition, it follows the the derivative of hydraulic head also follows the principle of super position and that velocity vectors from each of the analytical elements can be added together.

Darcy's law can also be written in terms of discharge potential as $q_l = - K_{sat} / (2h) \times \partial \phi/\partial l$, the derivative of discharge potential also follows the principle of superposition. This means that flow direction can be calculated analytically as the sum of flow direction components from all analytical elements.

Given flow in the direction of $l$, the flow velocity is equal to $v_l = q_l / n$, where $n$ is porosity of the aquifer.
...
### Case 1: Locating the source of urban contamination (web app)

A municipality receives complaints about the quality of its municipal water supply. Testing data suggests the possibility of groundwater contamination; the municipality wishes to establish, quickly and inexpensively, the radius of influence around its wells in order to determine the possible points of origin of the contamination. In this example, W1, W2, and W3 are municipal wells, each pumping 5 MGD (0.22 cumec); W1 is the contaminated well. Around these wells are [five?] sources of discharge: D1 and D2, which are injection wells which each operate at 0.45 MGD (0.02 cumec); and D3, D4, and D5, which represent points of known surface discharge. The area of interest is bounded by a no-flow boundaries to the [north, west, and south] and a [river/lake] to the east, represented by a constant-head boundary. Notice that several of the discharge points are between the features of interest in the model (the municipal wells and the constant-head boundary) such that the municipality, at the outset, is plausibly uncertain as to the relative effects of pumping at W1 and W2 and of the constant-head boundary.

![Figure X. Well capture of contamination by a municipal well, as demonstrated by anem-app.](../shiny-app/screenshots/urban_contamination.png)


#### Education opportunity

Can you design a plan for capturing the contamination, using the minimum amount of pumping? This will involve adding pumping wells to the map and ensuring none of the lines touch the main city well.

Each well costs \$1000, and the cost of pumping is \$3/cumec/well. 

What about doing this in a random way? i.e., generating random scenarios of contamination and allowing kids to play around with it.

### Case 2: Application for pumping well (web app and R package)
...
#### Web application -- approval of well
