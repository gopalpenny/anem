...
```{r flowdir_confined, message=FALSE, warning=FALSE}
...
```{r imaging}
well_images <- generate_image_wells(wells,boundaries,num_levels=5) %>%
  mutate(roi=get_radius_of_influence(Ksat=0.0001,h=50,t=3600*24*365*20,n=0.4,method="aravin-numerov"))
...
# now select only the wells that will affect the space
well_images <- well_images %>% filter(max_mirror_dist<roi)
well_roi <- gen_circles(well_images %>% rename(r=roi,x=x0,y=y0))
ggplot()  +
  geom_sf(data=boundaries, aes()) + 
  geom_sf(data=boundaries_utm, aes(), linetype="dashed",alpha=0.5,color="gray") + 
  geom_point(data=well_images,aes(x0,y0),alpha=0.5,size=3) + 
...
#   summarize(xmin=min(X),xmax=max(X),ymin=min(Y),ymax=max(Y))
grid_bounds_unconf <- data.frame(xmin=270000,xmax=290000,ymin=5113000,ymax=5120000)
...
ggplot(head_unconf_df %>% mutate(head_neutral=head_m==100)) + 
  geom_raster(aes(x,y,fill=head_neutral)) 
...
```{r flowdir_unconfined, message=FALSE, warning=FALSE}
grid_pts_flowdir_unconf <- with(grid_bounds_unconf,crossing(x=seq(xmin,xmax,length.out=75),y=seq(ymin,ymax,length.out=50)))
...
Now, plot the results together. 
```{r plot_unconfined,fig.width=6,fig.height=4}
ggplot(head_unconf_df) +
  geom_raster(aes(x,y,fill=head_m))  +
  geom_sf(data=boundaries, aes()) + 
  geom_sf(data=boundaries_utm, aes(), linetype="dashed",alpha=0.5,color="gray") + 
...
```{r check_bounds}
bounds_behavior_noim <- get_bounds_behavior(wells,boundaries,h0=100,Ksat=0.00001,aquifer_type="unconfined") %>% mutate(im="no_images")
bounds_behavior_wim <- get_bounds_behavior(well_images,boundaries,h0=100,Ksat=0.00001,aquifer_type="unconfined") %>% mutate(im="images")
bounds_behavior <- bind_rows(bounds_behavior_noim,bounds_behavior_wim) %>% 
  left_join(boundaries %>% st_set_geometry(NULL) %>% mutate(BT=paste(bID,bound_type)) %>% select(bID,BT),by="bID") %>%
  mutate(im=factor(im,levels=c("no_images","images")))
p_h <- ggplot(bounds_behavior) + geom_line(aes(dist_rel,head)) + facet_grid(BT~im,scales="free_x") + theme(axis.text.x=element_blank())
p_f <- ggplot(bounds_behavior) + geom_line(aes(dist_rel,flow_mag)) + facet_grid(BT~im,scales="free_x") + theme(axis.text.x=element_blank())


gridExtra::grid.arrange(p_h0,p_h1,p_f0,p_f1,nrow=1)
