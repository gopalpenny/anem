...
aquifer_recharge <- define_aquifer("confined",1,h0=0,z0=1,recharge=recharge_params,bounds=bounds_recharge)
...
ggplot() +
  ...
  geom_segment(data=gridded_2$flow,aes(x,y,xend=x2,yend=y2),
               arrow = arrow(ends="last",type="closed",length=unit(1,"mm")),color="black") +
  geom_segment(data=aquifer_recharge$bounds,aes(x1,y1,xend=x2,yend=y2,color=bound_type)) +
...
The package implements particle tracking in two ways: (1) tracking individual particles using `track_particles` and (2) estimating capture zones of wells using `get_capture_zone`, which initializes particles in the vicinity of wells and tracks them in reverse. See the help files for each function for more details.

### Integrating with anem-app

The basic functionality of this package is implemented online in a Shiny application, [anem-app](https://gopalpenny.shinyapps.io/anem-app/). The Shiny application provides an GUI for setting aquifer properties, drawing boundaries, defining recharge, adding wells, and tracking particles. It also displays results directly in the app. You can also download scenarios created on anem-app and import them into R. To do so, check out `?import_app_rds` from this package to learn more.
